...
class napari_focus_gui:
    ...
    def fit_widget(self):
        @magicgui(call_button = 'Fit Erfs',
                show_erfs = {'label':'Show Error Function Fits', 'value':False}
                )
...
def fit_x_focus(crop_image, show_erfs: bool = False):
    ...
    for i in range(n_curves):
        ...
        data -= 0.5
        data *= 2
        amplitude_init = np.max(data) - np.min(data)    # amplitude
        x0 = [  
                0,      #offset
                1.0,    # amplitude
                0.0,    # center
        ...
        out.append(least_squares(partial(residual, data = data, x = x_arg), x0 = x0, bounds = bounds).x)
        if show_erfs:
...
def erf_fit(params, x = []):
    ...
    diffarg = -0.5 * amplitude * np.exp(-argx * argx) / (np.sqrt(2.0) * sigma)
