...
# Copyright (C) 2024  M. Cyrus Daugherty -> Translation to cupy for GPU rvt
...
def generate_all_kernels(rmin, rmax, coarse_factor=1, coarse_mode="add"):
    ...
    if coarse_factor>1:
        if coarse_mode=="skip":
            ...
            kernels=[cp.sum(kernels[i:i+coarse_factor],axis=0)/coarse_factor for i in range(0,len(kernels),coarse_factor)]
    return kernels
...
def prepare_fft(inp, fshape, pad_mode="constant"):
    ...
    if pad_mode=="fast":
        return cp.fft.rfftn(inp,fshape)
    else:
...
def convolve_fft(sp1, sp2, s1, s2, fshape, fast_mode=False):
    """
    Calculate the convolution from the Fourier transforms of the original image
    and the kernel, trimming the result if necessary
    """
    ret=cp.fft.irfftn(sp1*sp2,fshape)
    if fast_mode:
        return cp.roll(ret,(-(s2[0]//2),-(s2[1]//2)),(0,1))[:s1[0],:s1[1]].copy()
    else:
...
def rvt_core(img,
    ...
    s1=cp.array(img.shape)
    s2=cp.array([rmax*2+1,rmax*2+1])
    ...
    rmeans=cp.array([convolve_fft(img_fft,k_fft,s1,s2,fshape,fast_mode=fast_mode) for k_fft in kernels_fft]) # calculate M_r for all radii
    if rweights is None:
        vom=cp.var(rmeans,axis=0) # calculate VoM as a standard variance of M_r along the radius axis
    else:
        vom=cp.sum(rmeans**2*rweights[:,None,None],axis=0)-cp.sum(rmeans*rweights[:,None,None],axis=0)**2 # calculate VoM as a weighted variance of M_r along the radius axis
    if kind=="basic":
        return vom
...
def high_pass(img, size):
    ...
    return img-ndimage.gaussian_filter(img,size)
...
def rvt(img, rmin, rmax, kind="basic", highpass_size=None, upsample=1, 
    ...
    Args:
        ...
        pad_mode: edge padding mode for convolutions; can be either one of modes accepted by ``cp.pad`` (such as ``"constant"``, ``"reflect"``, or ``"edge"``),
