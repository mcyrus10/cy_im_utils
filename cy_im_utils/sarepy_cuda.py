...
from cupyx.scipy.ndimage import (gaussian_filter, median_filter,
                                 binary_dilation, uniform_filter1d)
...
def remove_large_stripe_GPU(sinogram: cp.array,
    ...
    list_mask = [cp.array(binary_dilation(list_mask[i], iterations=1),
                          dtype=cp.float32) for i in range(n_sino)]
    ...
    # Make this into a CUDA Kernel? <-- THIS LOOP EXECUTES SUPER QUICKLY, I
...
def remove_stripe_based_filtering_sorting(sinogram,
    ...
    pass the gaussian window instead of calculating it inside the function (for
    now, probably simple to implement)
...
def detect_stripe_GPU(list_data: cp.array,
    ...
    slope, intercept = cp.polyfit(listx[ndrop:-ndrop-1],
                                  list_sort[:, ndrop:-ndrop-1].T, 1)
...
def remove_all_stripe_GPU(sinogram: cp.array,
    ...
    sinogram = remove_unresponsive_and_fluctuating_stripe_GPU(sinogram,
                                                              snr,
                                                              la_size)
    sinogram = remove_large_stripe_GPU(sinogram, snr, la_size, drop_ratio,
                                       norm)
