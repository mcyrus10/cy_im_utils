...
def image_array(image_array: np.array,
    ...
    # this conditional selects a compatible slicing function for the array
    # shape
    if ndim > 2:
        def array_slice_fun(i, j):
            return image_array[i, j, ...].astype(np.float32)
    else:
        def array_slice_fun(i, j):
            return image_array[i, j].astype(np.float32)

    fig, ax = plt.subplots(n_row, n_col, figsize=figsize, sharex=sharex,
                           sharey=sharey)
...
def COR_interact(data_dict: dict,
    ...
    # Create the combined image in attenuation space to determine center of
    # rotation
    n_angles = len(angles)
    ...
    def inner(crop_y0, crop_dy, crop_x0, crop_dx, norm_x0, norm_dx, norm_y0,
              norm_dy, cor_y0, cor_y1, tpose):
        if tpose == 'True':
        ...
        l, h = np.quantile(distribution, dist), \
            np.quantile(distribution, 1.0-dist)
        ...
        if y0 == y1:
            ...
            if (norm_patch[0] != norm_patch[1] and
                ...
                ax[0].text(norm_patch[0], norm_patch[2],
                           ...
                           rotation=90)
            ax[1].axis(False)
        elif (y0 != y1 and crop_patch[0] != crop_patch[1]
            ...
            if y1 > crop_patch[3]-crop_patch[2]:
                ...
                if (norm_patch[0] != norm_patch[1] and norm_patch[2] !=
                        norm_patch[3]):
                    ax[0].text(norm_patch[0], norm_patch[2],
                               'Norm Patch',
                               color='w',
...
def median_interact(data_dict: dict,
    ...
    def inner(frame, median_z, median_x, median_y, row):
        ...
        if frame - median_z_stride <= 0:
            ...
            filtered = median_gpu(
                                cp.array(input_array[slice_z].copy()),
                                med_kernel
                                )[median_z_stride, :, :].get()
...
def median_2D_interact(data_dict: dict,
    ...
    def inner(median_1, thresh_kernels, thresh_z_scores, xlim, ylim):
        ...
        if median_1 > 1:
            ...
            ax[1].axis(False)
        if thresh_kernels != "" and thresh_z_scores != "":
...
def TV_interact(data_dict: dict,
    ...
    """
    Interactive inspection of TV-POCS with sliders to control the arguments.
    The data_dict can
...
def dynamic_thresh_plot(im,
    ...
    """
    This is still a work in progress, I can't figure out how to curry the
    interactive plot... :(
    ...
    def innermost(thresh_0, thresh_1, thresh_2):  # , thresh_3):
        ...
        for i in range(n_interval):
            ...
            ax.append(fig.add_subplot(gs[0, hist_width+i+1], sharex=ax[-1],
                                      sharey=ax[-1]))
...
def volume_register_interact(static: np.array,
    ...
    def inner(x_coord, y_coord, z_coord, x_shift, y_shift, z_shift,
              theta_yz, theta_xz, theta_xy, scale, alpha):
        plt.cla()
        [a.clear() for a in ax]
