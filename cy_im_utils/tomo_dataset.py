...
to do:
    - batch fdk cone
    - make sure jupyter version still works correctly 
        - update center of rotation sub-routine
...
class napari_tomo_gui(tomo_dataset):
    ...
    def cor_wrapper(self):
        ...
        def inner():
            ...
            self.settings['crop'] = {   'y0':int(y0),
                                        'y1':int(y1),
                                        'x0':int(x0),
                                        'x1':int(x1)}
            self.settings['COR'] = {'y0':int(cor_y0),'y1':int(cor_y1),
    ...
    def sare_widget(self):
        ...
        @magicgui(call_button = 'Preview Ring Filter',
                row={'label':'Sinogram Index (row)','value':0,'min':0,'max':10_000},
                snr={'value':snr_init},
