...
class astra_tomo_handler:
    ...
    def __init__(self, settings_dict: dict):
        ...
        self.algorithms_2d = ['FBP_CUDA', 'SIRT_CUDA', 'CGLS_CUDA', 'EM_CUDA',
                              'SART_CUDA']
    ...
    def _parse_settings_3D(self,
        ...
        if geometry == 'parallel3d':
            ...
            print('source detector:', settings['source to origin distance'])
            print('origin detector:', settings['origin to detector distance'])
            # print('params:\n', proj_args)
        else:
    ...
    def astra_reconstruct_3D(self,
        ...
        # print("--> projeciton args = ", proj_args)
        proj_geom = astra.create_proj_geom(*proj_args)
    ...
    def reconstruct_volume(self,
        ...
        if self.geometry in self.geometries_2d and \
            ...
            for j in tqdm(range(nsino), desc='reconstrucing volume'):
                recon[j] = self.astra_reconstruct_2D(
                                                sinogram_volume[j],
                                                angles=angles,
                                                settings=self.settings)
            ...
            assert False, f"{self.geometry} incompatible with {self.algorithm}"
...
def ASTRA_General(attn: np.array,
    ...
    known_algos = ['FDK_CUDA', 'SIRT3D_CUDA', 'CGLS3D_CUDA', 'FP3D_CUDA',
                   'FP3D_CUDA']
...
def TV_POCS(sinogram: np.array,
    ...
    if angles is None:
        angles = np.linspace(0, 2*np.pi, sinogram.shape[0], endpoint = False)
    ...
    # for j in tqdm(range(num_iter)):
        if debug:
            print(f"iteration {j} -----------------------------")
        f0 = f.copy()
        f += beta*astra_back_project_local_function(g0-g,
                                                    algorithm=algorithm,
        ...
        if dg > r_max*dp and dd > eps:
            dtvg *= alpha_red
            if debug:
                print(f"iter {j}: stepping down gradient descent parameter")
        beta *= beta_red
        if debug:
            print("beta = ", beta)
